...
sp_new = []
for i in sp:
    if (int(i)>0 and (float.is_integer(math.sqrt(int(i))))):
        sp_new.append(int(math.sqrt(int(i))))
print(sp_new)
print('*'*50)
...
if ((31>=int(date[0])>=1) and (12>=int(date[1])>=1)):
    date_txt='{d} {m} {y} года'.format(d=days[date[0]], m=months[date[1]], y=date[2])
    print(date_txt)
...
print('Задача-3')
import random
count = int(input('Введите количество чисел для заполнения списка: '))
sp=[]
...
print('Задача-4')
sp = input('Ведите числа через пробел: ').split(' ')
sp_new = set(sp)
print(sp_new)
